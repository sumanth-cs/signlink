...
try:
    ...
    print("Warning: 'enchant' C library not found. Spell-check suggestions will be disabled.")
    class MockEnchant:
        def check(self, word): return True
...
os.environ["THEANO_FLAGS"] = "device=cpu, floatX=float32, optimizer=None"
...
class Application:
    ...
    def __init__(self):
        ...
        self.panel.place(x=100, y=3, width=480, height=440)
        ...
        self.b1.place(x=390,y=580)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=590, y=580)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=790, y=580)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=990, y=580)
    ...
    def speak_fun(self):
        ...
        languages = {'kn': 'Kannada', 'hi': 'Hindi'}

        for lang_code, lang_name in languages.items():
            # Translate
            translated = translator.translate(inputstr, src='en', dest=lang_code)
            print(f"\nOriginal: {inputstr}")
            print(f"Translated to {lang_name}: {translated.text}")
            # Select script for transliteration
            if lang_code == 'hi':
                ...
                target_script = None
            if target_script:
                output = transliterate(translated.text,target_script, sanscript.ITRANS)
                print(f"Transliterated to {lang_name} script: {output}")
                engine = pyttsx3.init()
                engine.say(output)
                engine.runAndWait()
